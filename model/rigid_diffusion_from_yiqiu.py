######################################################################################
#不实际使用，是一个temp文件，这里是更新了 pair embedding的更新方式，但有bug。
######################################################################################
...
from model.pair_embedding_v2_origg import PairEmbedder
...
class EdgeUpdate(nn.Module):
    ...
    def forward(self,
        ...
        orientation_quaternions = quaternions(orientation)
        pair_feature = torch.cat((distance_rbf, altered_direction, orientation_quaternions), dim=-1)
        x = pair_feature.float()

        x = self.linear_1(x)
        x = self.relu(x)
        x = self.linear_2(x)

        pair_emb = x + pair_time + relative_pos
...
class EdgeTransition(nn.Module):
    ...
    def forward(self, node_emb, edge_emb, updated_pair_features):
        ...
        d_emb = self.distance_linear(updated_pair_features)
...
class RigidDiffusion(nn.Module):
    ...
    def forward(self,
        ...
        #print("============pred_chi_sin_cos=================", pred_chi_sin_cos.shape)

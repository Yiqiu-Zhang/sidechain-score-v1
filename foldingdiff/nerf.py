...
def nerf_build_batch_O(
    ...
    N-CA-C atoms.
    """
    #print('======================bond_angle_n_ca_c======================',bond_angle_n_ca_c)
    #print('======================bond_angle_ca_c_n======================',bond_angle_ca_c_n)
    #print('======================bond_angle_c_n_ca======================',bond_angle_c_n_ca)
     
    assert phi.ndim == psi.ndim == omega.ndim == 2  # batch, seq
    assert phi.shape == psi.shape == omega.shape
    batch = phi.shape[0]

    # (batch, 3(# atom), 3(3D coord))
    coords = (
        torch.tensor(np.array([N_INIT, CA_INIT, C_INIT]), requires_grad=True)
        .repeat(batch, 1, 1).to(phi.device)
    ...
    for i in range(phi.shape[1]):
        ...
        bond_angle_ca_c_o = 120.1 / 180 * np.pi
        bond_len_ca_c_o = 1.229 
        # Place the C-O
        o_coord = place_dihedral(
            coords[:, -3, :],  # after indexing, shape is (batch, 3)
            coords[:, -2, :],
            coords[:, -1, :],
            #bond_angle=bond_angle_ca_c_o,
            #bond_length=bond_len_ca_c_o,

            bond_angle=bond_angle_ca_c_n[:, i].unsqueeze(1), #临时
            bond_length=bond_len_c_n[:, i].unsqueeze(1),#临时
        ...
        coords = torch.cat(
            [coords, o_coord.unsqueeze(1), n_coord.unsqueeze(1), ca_coord.unsqueeze(1), c_coord.unsqueeze(1)],
            dim=1,
        )
        assert coords.shape[-1] == 3 and coords.shape[0] == batch

    coords = torch.cat([coords, c_coord.unsqueeze(1)], dim=1,) # !!!!!!!! 需要添加C端O院子
